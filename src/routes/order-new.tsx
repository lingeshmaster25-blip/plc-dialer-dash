...
function OrderNewPage() {
  ...
  const [modal, setModal] = useState<null | { kind: "success"; no: number } | { kind: "error" }>(null);
  ...
  const handleConfirm = () => {
    if (!previewReady) { setModal({ kind: "error" }); return; }
    setModal({ kind: "success", no: orderCounter });
    orderCounter += 2;
  };
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontSize: 30, fontWeight: 600, color: "#1a1a1a", textAlign: "center", lineHeight: 1.35 }}>
                {modal.kind === "success"
                  ? `Your Order #${modal.no} has been added to the Picklist Queue`
